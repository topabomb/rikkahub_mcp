...
- 企业公开定义由聊天助手选择与当前空间设置呈现，运行连接及凭据不可披露。当前 Bridge v3 没有 Starter/打开聊天方法；工作台的原生宿主提供常用入口，不向网页新增数据库、配置或聊天写权限。
...
| 企业助手查看 | 企业定义中的模型作为助手默认模型，提示词/MCP、子助手绑定和 Seed 保持受管只读；当前空间模型覆盖与运行记忆通过聊天中的统一助手设置管理，不为查看配置创建持久会话，不把企业 ID 交给个人定义编辑器 |
...
- 当时的企业助手目录曾验证公开配置、只读 Seed 与运行记忆 CRUD；该目录现已退休，当前入口为聊天助手选择与当前空间设置，Seed 与运行记忆仍沿同一 owner 和授权协议。
...
标准 memory_tool 成功创建记录；强制停止并重启应用后，当时从企业助手目录仍可读取该助手的运行记忆，企业写作 Seed 单独保留。该历史界面已退休，证据仍为 `experience-release-*.xml`、`experience-release-memory.png`。
