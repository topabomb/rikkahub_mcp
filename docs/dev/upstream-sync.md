...
### 2026-07-11 - 检查 8e872e5f+ 更新（第八批）

> **同步状态：✅ 全部完成（编译通过 + 单元测试通过 + 逐文件核对一致）**
>
> 详细分析文档：[batch-8-2026-07-11.md](upstream-sync/batch-8-2026-07-11.md)

**检查范围**：`8e872e5f..upstream/master`（2026-07-09 ~ 2026-07-11，共 11 个提交）

**原项目信息**：
- Fork 基线：2.3.1（versionCode 164）- 2026-06-18
- 本次检查最新上游提交：`1ff465d1`（2026-07-11）
- 上次检查时间：2026-07-08（第七批，`8e872e5f`）
...
| # | 提交 | 描述 | 类别 |
|---|------|------|------|
| 1 | `b6b16842` | TTS 不朗读括号内容 + removeBracketedContent | 功能增强 |
| 2 | `449ce1e6` | removeBracketedContent 单元测试 + TTS 括号设置本地化 | 测试补充 |
| 3 | `d910c4a1` | WebView 预览内容缓存到文件避免 TransactionTooLargeException | bug 修复 |
| 4 | `dab4e448` | 修正 TTS 本地工具描述为“所选 TTS 服务” | i18n 修复 |
| 5 | `40592c1b` | 恢复备份后目录缺失改为标记 BROKEN 而非删除记录 | bug 修复 |
| 6 | `d37d4ed6` | 优化建议回复默认提示词 | 提示词优化 |
| 7 | `0dfb598c` | 补全中英文 TTS 引号描述显示 | i18n 修复 |
| 8 | `1ff465d1` | 自定义 JS 搜索 scrape 参数补全 array items 声明 | bug 修复 |

**跳过（3 个提交）**：

| 提交 | 描述 | 原因 |
|------|------|------|
| `9d337938` | fix stale upstream unit tests | 已同步（ChatCompletionsAPIMessageTest 已有 3 参数签名；build.gradle.kts 已有 testImplementation；MiMo 测试已是 v2.5；ElevenLabs/FishAudio 测试不存在于 fork） |
| `3c1a2de6` | 捕获 OEM 拒绝 FGS 权限导致 Web 服务器启动闪退 | Fork 未引入 Web 模块 |
| `cf55cbbb` | CI 仓库拥有者 Issue 跳过空白模板自动关闭 | CI 配置，不适用 |
...
| 10 | `0217ca82` | imggen ImageGenSize 重构 | ✅ 同步 | P1 |
| 11 | `c66935f2` | 移除重复模型选择器 | ✅ 同步 | P1 |
| 12 | `e1ef629b` | TTS 引号扩展（直角引号） | ✅ 同步 | P1 |
| 13 | `57e6dbc1` | 直角引号提取测试 | ✅ 同步 | P1 |
| 14 | `5f5f9654` | 状态栏沉浸修复（助手详情/设置页） | ✅ 同步 | P1 |
| 15 | `5ea0e35f` | 扩展管理按钮关闭面板修复 | ✅ 同步 | P1 |
| 16 | `624ab635` | 工作区导入按钮移至顶栏 | ✅ 同步 | P1 |
...
| # | 提交 | 描述 | 判定 | 优先级 |
|---|------|------|------|--------|
| 1 | `7d88bbc3` | 流式丢字修复 + replaceRegexes 缓存 + 通知拆分到 ChatNotificationManager | ✅ 同步 | P0 |
| 2 | `8477334d` | MCP SSE 通知流重试耗尽不再触发整体重连 | ✅ 同步 | P0 |
| 3 | `55c2cf71` | 聊天头部模型名支持两行显示 | ✅ 同步 | P1 |
| 4 | `8e872e5f` | 聊天输入框键盘收起时增加底部呼吸间距 | ✅ 同步 | P1 |
| 5 | `6a35f89e` | TTS 语气标记引导 + MiMo 接入 | ✅ 同步 | P2 |
