...
> 本项目已经过大量架构和约束重构，数据库以及存储持久化也有所变更，fork时上游项目架构参见`docs\dev\original-architecture.md`，当前项目架构参见 `docs\references\application-architecture-v1.md` ,引入上游变更时充分考虑到架构、代码文件的变化，同时要遵照本项目的架构和约定来规划引入方案。
...
6. 审查所有同步的变更的所有代码的正确性、完整性以及语义、流程清晰、分层和边界清晰、职责明确。
...
### 2026-08-17 - 检查 f6017cbb+ 更新（第十二批）

> **同步状态：✅ 全部完成（13 个同步 + 3 个按需引入 + 4 个跳过；相关单元测试与 assembleDebug 通过；版本号不变更）**
>
> 详细分析文档：[batch-12-2026-08-17.md](upstream-sync/batch-12-2026-08-17.md)

**检查范围**：`f6017cbb..693c2ce5`（2026-08-13 ~ 2026-08-16，共 20 个提交）

**原项目信息**：
- Fork 基线：2.3.1（versionCode 164）- 2026-06-18
- 本次检查最新上游提交：`693c2ce5`（2026-08-16）
- 上次检查时间：2026-08-13（第十一批，`f6017cbb`）

**版本号对应关系**：

| 维度 | 上游 | 本地 | 说明 |
|------|------|------|------|
| 版本名 | 2.4.10 | 0.0.16 | 版本线独立，本批不递增 |
| versionCode | 177 | 16 | 独立递增，本批不变更 |
| DB 版本 | v24+ | v4 | 本批无 Room schema 变更 |

**需同步（13 个提交）**：

| # | 提交 | 描述 | 判定 | 优先级 |
|---|------|------|------|--------|
| 1 | `5b390510` | 更新提醒可暂停，保留 showUpdates | ✅ 同步 | P1 |
| 2 | `aac6e963` | 内置搜索抑制外挂搜索工具 | ✅ 同步 | P0 |
| 3 | `8824e0e8` | 保留 Serializable keep | ✅ 同步 | P0 |
| 4 | `9adc82ec` | jlatexmath 整包 keep | ✅ 同步 | P0 |
| 5 | `f55c8c14` | 流式 null choices/delta/tool_calls | ✅ 同步 | P0 |
| 6 | `edbd5773` | ReasoningLevel.MAX，按本地 profile 映射 | ✅ 同步 | P1 |
| 7 | `0c52b62b` | 加密思考不回放明文 | ✅ 同步 | P0 |
| 8 | `d1e8effc` | 推理选择器去掉底部刻度 | ✅ 同步 | P2 |
| 9 | `de888df2` | DashScope ASR realtime / session.finish | ✅ 同步 | P0 |
| 10 | `97df86ec` | SAF queryDocument 检查存在性 | ✅ 同步 | P1 |
| 11 | `bca21d4d` | 允许 .agc 附件 | ✅ 同步 | P2 |
| 12 | `82758c36` | 英文句首自动大写 | ✅ 同步 | P2 |
| 13 | `dca7f01c` | 正则列表可排序 | ✅ 同步 | P1 |

**按需引入（3 个提交）**：

| 提交 | 描述 | 判定 | 原因 |
|------|------|------|------|
| `c077e0c8` | 更新依赖 | ⚠️ 按需引入 | 引入 Compose/Nav/MCP/Haze 等；跳过 Firebase 与本地没有的 nav3Material |
| `7d914e60` | 禁用重命名混淆 | ⚠️ 按需引入 | 不引入 `-dontobfuscate`，保留本地混淆 + mapping |
| `9b964ca0` | 文生图 Provider Override | ⚠️ 按需引入 | 运行时已正确；补 `FindProviderTest` |

**跳过（4 个）**：

| 提交 | 描述 | 原因 |
|------|------|------|
| `04767aa7` | bump 2.4.7 | 版本线独立 |
| `8dc3ebce` | bump 2.4.9 | 版本线独立 |
| `c88822d6` | 豆包搜索 | 新搜索引擎，与 Fork 精简冲突 |
| `693c2ce5` | bump 2.4.10 | 版本线独立 |

> 关键对照：暂停更新提醒必须保留 `showUpdates` 和单例 `UpdateChecker`；MAX 只在 DeepSeek 等接受 `max` 的端点发送 `max`；加密思考回放不能带明文 summary。

### 2026-08-13 - 检查 b106e8bb+ 更新（第十一批，同日扩展）

> **同步状态：✅ 全部完成（8 个同步 + 1 个按需引入 + 3 个跳过；相关单元测试通过；版本号不变更）**
>
> 详细分析文档：[batch-11-2026-08-13.md](upstream-sync/batch-11-2026-08-13.md)

**检查范围**：`b106e8bb..f6017cbb`（2026-08-02 ~ 2026-08-13，共 12 个提交）

**原项目信息**：
- Fork 基线：2.3.1（versionCode 164）- 2026-06-18
- 本次检查最新上游提交：`f6017cbb`（2026-08-13）
- 上次检查时间：2026-08-02（第十批，`b106e8bb`）

**版本号对应关系**：

| 维度 | 上游 | 本地 | 说明 |
|------|------|------|------|
| 版本名 | 2.4.6 | 0.0.15 | 版本线独立，本批不递增 |
| versionCode | 173 | 15 | 独立递增，本批不变更 |
| DB 版本 | v24+ | v4 | 本批无持久化 schema 变更 |

**已同步（8 个提交）**：

| # | 提交 | 描述 | 类别 |
|---|------|------|------|
| 1 | `fac1c6c8` | 注册 qwen-3.8-max | 新模型适配 |
| 2 | `4b6449e3` | 注册 mimo v3 / v3 pro | 新模型适配 |
| 3 | `2c980642` | WebView onRelease 真正销毁 | bug 修复 |
| 4 | `aa487c8c` | Claude Sonnet 5 / Opus 5，并接入本地 adaptive thinking | 新模型适配 |
| 5 | `04d6c4ea` | muse-spark / muse-glimmer + Meta 图标 | 新模型适配 |
| 6 | `1af15299` | grok 图标改为 xai.svg | UI |
| 7 | `fdc0bc9a` | 内置搜索按 Provider 开门 | bug 修复 |
| 8 | `8c3f8240` | 搜索选择改为 SearchMode 卡片 | UI |

**按需引入（1 个提交）**：

| 提交 | 描述 | 判定 | 原因 |
|------|------|------|------|
| `4b9ff1a9` | AI 模块流式重构 | ⚠️ 按需引入 | 不整包替换流模型；吸收图片 data URI 源头完整性，以及 OpenRouter host 限定的 `reasoning_details` |

**跳过（3 个）**：

| 提交 | 描述 | 原因 |
|------|------|------|
| `3c9457b4` | DeepSeek Chat effort 映射 | 本地已按 Chat/Responses 分端点映射，且不把 XHIGH 写成 `max` |
| `a00e576a` | 缩短 AGENTS.md、删除 .editorconfig、升 2.4.6 | Fork 自有 AGENTS / 保留 editorconfig；版本线独立 |
| `f6017cbb` | `.claude` 改为指向 `.agents` 的符号链接 | 本地 agent 工作流独立，不引入上游 `.claude` 变更 |

> 关键对照：上游解决解码与聚合的结构债务，本地解决回放与运行时正确性债务。
> 本批不整包替换为 `StreamChunk`。将来演进必须记住的分层原则见详细文档第七节。
...
*最后更新：2026-08-17（第十二批同步完成，`f6017cbb..693c2ce5` 共 20 个提交，13 同步 + 3 按需 + 4 跳过；相关单元测试与 assembleDebug 通过；版本号不变更）*
