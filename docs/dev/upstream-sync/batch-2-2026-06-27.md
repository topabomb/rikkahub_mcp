# 上游同步分析 — 第二批（2026-06-27）

> 本文档为第二批上游同步检查的完整分析记录，包含逐提交 diff 分析、本地代码核对、适配要点。
...
- **本次检查范围**：`5b9be301..a6e7a305`（2026-06-18 ~ 2026-06-26，共 21 条提交）
- **本次检查最新上游提交**：`a6e7a305`（2026-06-26）
- **上次检查时间**：2026-06-20（第一批，已同步 2.3.2 的 OCR 修复 / 平板 UI 修复 / 依赖更新）

**核对方法**：逐一获取上游 commit 完整 diff（`.patch` 原始 URL），与本地实现逐行比对。

---

## 二、跳过（6 个提交 — 与 Fork 精简方向冲突）
...
- **`LocalToolOption.ScreenTime`**：`@SerialName("screen_time")` 新增枚举，旧 `settings.json` 无此字段时默认不启用，**无迁移风险**
- **Room 数据库**：本次同步不涉及 Room entity 变更，**无需 Migration**
- **权限新增**：`PACKAGE_USAGE_STATS` 为运行时权限，首次开启屏幕使用时间工具时通过 PermissionManager 框架申请，**不影响已安装用户**
