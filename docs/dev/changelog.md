...
## 0.0.19（versionCode 19）— 2026-08-26 ~ 2026-08-29

### 新增

- MCP 引入独立的服务器运行时与持久化工具目录：已确认的非空目录可跨重启恢复，新会话使用稳定快照，服务暂时离线时工具仍可见
- MCP 设置页与会话工具选择器补齐按服务重试、下拉刷新、连接状态说明和设置入口；移动端前后台及网络恢复共用统一生命周期
- 企业配置、会话错误、附件预览与多模态请求增加明确的读取边界和诊断投影
- 聊天生成切到后台后可继续运行，并通过前台通知提供返回入口
- 文件管理支持按 7、14 或 30 天清理旧上传文件和生成图片
- 关闭 Workspace 终端标签前增加确认，避免误关仍在使用的会话

### 变更

- MCP 配置只保存服务定义和用户策略，远端工具目录不再混写 Settings；连接、发现、通知刷新、手工刷新、重试与调用统一由每个服务器的单一运行时负责
- MCP 工具目录纳入 v4 手工备份；旧 Settings 与 v3 备份中的完整目录会一次性迁移，不保留历史读取旁路
- MCP 启动改为按需激活和有限并发，短时快速恢复后进入适合移动端的后台维护重试；刷新只更新后续轮次，当前轮次的工具前缀保持稳定
- MCP 工具失败结果收敛为简洁、无歧义的协议；远端明确返回的错误内容继续透传，不再向 Agent 暴露内部状态机字段
- 会话、Settings、Artifact、工具阶段和多模态能力继续按 V2 单一 owner 收敛，删除旧 facade、重复写路径和失效协议
- 图片能力由模型配置统一决定；审批继续沿用原 Turn 和既有工具事实，不因运行时可用性变化创建第二条执行链
- Token 用量统一按 Provider 实际返回统计；聊天底部紧凑显示最近请求的 Context、缓存占比和请求数，高缓存命中率提供更细精度，可展开查看累计量与响应速度，主助手与子助手保持独立
- MCP 浏览器授权改为本机一次性回调，授权完成后可直接返回应用，并提升并发授权时的稳定性
- 模型选择器在搜索键盘弹出时保持打开；修正 Qwen 3.8 的视觉能力标识，并更新 Kimi 图标

### 修复

- 修复 MCP 已连接但工具目录未可靠提交、多个服务互相阻塞、刷新完成不可判断以及短暂断连后工具从 Agent 上下文消失的问题
- 修复 MCP 分页发现、空目录误发布、服务端工具变更通知、同名配置更新、OAuth 并发回调及断连恢复中的竞态；连接传输、地址或静态请求头变化时不再沿用旧 OAuth 凭据
- 修复附件检查能力协商、Provider 不完整诊断、会话取消恢复和 Settings 图片悬空引用等一致性问题
- 修复多步工具调用、流式响应及缓存命中场景中的 Token 重复或漏计；统计页恢复缓存命中输入量，并提示旧版或不完整记录
- 修复仅包含工具结果的消息被误判为空、系统注入内容错误套用用户模板的问题
- 修复部分无输入的 Workspace 命令一直等待，以及旧文件清理后显示结果不准确的问题
