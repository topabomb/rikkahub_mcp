...
- 文件管理增加「文生图」Tab
- **多图查看器**：点击图片进入全屏查看器并左右翻页——聊天为会话级时序相册（当前分支全部用户附件与工具产物，跨消息导航），文生图当次结果与 Gallery、文件管理两个 Tab 为各自分组；多图显示页码；支持点按关闭与未放大时竖直拖拽关闭。详见 [image-viewer-upgrade-plan.md](image-viewer-upgrade-plan.md)
- 查看器底部新增图片信息按钮（下载按钮左侧，整组居中）：面板展示来源/文件名/分辨率/大小/格式/修改时间，点外部或返回键关闭，翻页自动刷新
- 删除确认补充上下文：文件管理单删/清空与 Gallery 删除均弹确认，文案含文件名或 prompt、来源（模型）与后果（是否影响聊天/背景副本）；清空提示含数量
...
- 文生图结果区按当次生成全量展示（1–4 张，两两一行），不再截断为 2 张
- 文件管理 Upload 的图片与文生图 Tab 产物可点开查看器；非图片维持原状

### 修复

- 文生图编辑走共享队列；取消背景更新不再被改写成普通失败
- 清空 Upload 会提示同时删掉聊天副本和助手背景
- 查看器保存 Toast 硬编码中文改为本地化（各 locale），保存按钮补无障碍描述
- 文生图页 error Toast 收集器上移到页面级：原先停留在 Gallery 时删除失败的错误提示不会弹出
