...
| Chat 输入模板 | 当前企业助手的 Starter 只读预填；追加原草稿、保留附件，等待用户发送 |
| 工作台 | 固定 Portal 包信息/动态、相机/录音；关闭仅关闭网页，当前 Bridge 不提供 Starter 或打开聊天动作 |
...
从本计划创建提交的父提交至当前实现复审，发现并修正动态/历史/分页/统计/文件查询在读取中到期后仍返回结果、旧空间失败反馈、WebView 回调内销毁与动态编辑的滚动/键盘可达性问题。当前参考文档已对齐实际 owner 和消费者。具体差异、门禁及实际使用证据见 [本轮复审记录](android-enterprise-review-2026-09-11.md)。工作台 Starter、企业助手配置和记忆入口及全设置页的追加范围在本轮修复提交后单独推进。
