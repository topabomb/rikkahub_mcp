...
- 企业路由图片成功响应按已验证请求数与 20 MiB 单图上限计算有界 stream，并直接解码 typed 结果，避免原始正文和 JSON tree 双份驻留；非 2xx 复用 explicit-route owner 的 128 KiB 脱敏错误上限。真实 OkHttp transport 测试覆盖 DNS 固定、禁止自动重定向、分块超限中止和取消传播，成功媒体仍只交给 `GeneratedMediaStore` 持久化。
...
| Android JVM/构建 | `test assembleDebug lintDebug assembleRelease --no-parallel --max-workers=1` 通过，827 tasks，9m08s |
| Android 设备 | Pixel 10 Pro Fold API 37：Enterprise 页面正常态 16/16，包含合成竖向 separating hinge 的右 pane 几何断言；此前半开竖屏、半开横屏各 15/15，均 0 失败。完整 `connectedDebugAndroidTest` 的 app 217、speech 17、workspace 13 个测试全部完成，10 个既有 live/PRoot 前置条件跳过、0 失败 |
