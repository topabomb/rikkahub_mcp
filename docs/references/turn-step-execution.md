...
| `ConversationTurnService` | Master turn 输入、工具与副作用编排；不实现第二套持久化协议 |
...
| `ConversationRuntime` | 唯一事实流 `ConversationRuntimeSnapshot`（durable `ConversationAggregateSnapshot` + 纯内存 `TurnStreamProjection`）与私有 `ActiveTurnSession` 请求状态机。durable CAS 仍只用 `TurnHandle` |
...
| `TurnCommitter` / `TurnPipelineFactory` | Master/Target 共用的 start、checkpoint、stream 与非空 `TurnRunResult` 协议（终态 `TurnOutcome` / 暂停 `TurnPause`）；固定 Transformer 顺序 |
...
用户发送
  │
  ▼
ConversationApplicationService / ConversationTurnService
  ├─ 等待 ApplicationRecoveryGate.Ready
  ├─ 结束上一 turn，并收口需要保留的工具/Child 结果
  ├─ 预处理用户消息与稳定附件引用
  ├─ AppendUserMessage durable command：用户消息与首轮本地标题同事务
  ├─ 从一次 EffectiveSettingsSnapshot 捕获 prompt/disclosure/MCP/tools 与不可变 TurnContext
  └─ TurnCommitter.start
       └─ StartTurn 单事务：assistant 槽 + RUNNING turn fact + 可选 model-context entry
            │
            ▼
       TurnRunner.run：多 Step 循环（TurnRunState 持有跨 Step 草稿/累加器/未发布资源作用域）
         ├─ StepRunner.run 单 Step 采样：RequestContextPlanner.planRequest：replay-safe 后按完整 USER 轮次裁剪，只改变本次请求
         ├─ 稳定 System / frozen Tool prompt
         ├─ Input Transformers
         ├─ applyContextProjections：Snapshot 作为因果 USER 第一 part
         ├─ Provider stream
         ├─ Output Transformers
         └─ ToolBatchRunner.run：工具批次门控、审批暂停与串行执行 + awaited checkpoint
            │
            ▼
       TurnCommitter（loop 经 TurnRunInputs 同步回调各汇，无冷流 bind）
         ├─ onAssistantObserved：Producer 同步更新 TurnCommitter 唯一 Assistant 槽，不依赖 UI 交付
         ├─ onStreamDelta：TurnHandle/epoch 校验后把当前 Assistant 草稿更新进纯内存流式投影 `TurnStreamProjection`
         ├─ onCheckpoint：TurnCheckpoint durable command
         └─ onResult：commitRunResult 分派——TurnOutcome 终态落 FinalizeTurn durable command
              （TurnPause 落 AWAITING_USER ModelResponseCheckpoint，携带 pendingInteractions locator 列表，消费者不再回消息里扫描）
```

`ConversationTurnService.sendMessage` 在安装唯一 generation Job 时返回 `SendMessageReceipt`。receipt 中的
...
`StartTurn`、`TurnCheckpoint` 与 `FinalizeTurn` 均执行：
...
失败或取消时，Application 终态准备显式接收 TurnCommitter 累积的最新 Assistant，校验同一 `TurnHandle` 后在该 Assistant 上
关闭未完成工具；它不能改读 durable nodes，否则会丢失最后一次 checkpoint 之后已经流出的文本或工具 delta。
`TurnRunInputs.onAssistantObserved` 在每次发布前、特别是请求关闭 usage 后的可取消变换之前同步更新
`TurnCommitter` 的唯一 Assistant 槽；`onStreamDelta` 与 `onResult` 只消费展示和运行结果，不把迟到展示消息回写该槽。
...
`ModelResponseCheckpoint` 与带新输出的 `ToolResultCheckpoint` 在显示 local payload 前，先提交 checkpoint、
更新同一 Assistant 槽，再发布 lease。交接开始前检查取消；已经取得结果/资源所有权后的这一小段提交不可取消，
完成后继续传播取消。提交前取消或提交失败不会把随后被回滚的 local URI 交给终态消息；
checkpoint 成功后即使 lease 发布失败，终态仍使用已有 durable root 的消息，不退回提交前的内容。
Tool Result 的 output transformers 若生成受管文件引用，`ToolResultCheckpoint` 必须提交变换后的消息本身，再发布 lease；禁止用 raw result 建 durable checkpoint 却发布只被 presentation 引用的资源。
...
UI 状态表或第二写入口。loop 未走到正常完成时同样以 `Incomplete` 终态收口，不能遗留 RUNNING execution。
...
START 只建立一个进程内 `TurnContext`：它保存 resolved Assistant/Model、不可变 Provider wire shape、media capability、
`TurnPromptSnapshot`、有序 `FrozenToolDefinition` 与同名 execution bindings。`TurnRunInputs` 只携带该 context、消息、固定 pipeline
与流式/阶段/checkpoint/终态汇；Provider step、审批/`ask_user` continuation 与重试不得重读 Settings、Workspace、模板或时钟来改变模型可见请求。
...
`StepRunner.generateInternal()` 固定执行：
...
`ConversationTurnService` 通过 `TurnToolSetFactory` 在 START 前装配 Search、Local、Conversation、Workspace、Skill、
...
`AssistantToolFactory` 构造时必须同时获得 `SubAssistantRunCoordinator` 与 `TurnToolSetFactory`，
不把缺失装配降级为工具运行时错误。
...
同一 Assistant message 的 ToolCall 以 `ToolCallLocator(assistantMessageId, stepId, localCallId)` 定位；
ordinal 只是 accumulator 内的瞬时流式游标，绝不参与身份。审批是一批工具的屏障：Pending 全部解决后，
按 transcript 调用顺序串行执行。`ResolveToolInteraction` 在同一次 reducer 变换中更新 durable node 与 active-turn projection，
...
无效或已撤销的 Pending 调用清除等待态，并通过既有 `ToolResultCheckpoint` 提交 FAILED 结果；同批其余合法 Pending
仍形成审批屏障。`ToolResultCheckpoint` 的 RUNNING snapshot 与流式 Messages 不得包含新打上的 Pending：失败替换先
...
取消收口对尚无 Child link 的 `assistant_call` 使用通用中断结果：`TurnFinalizer` 根据已校验的 turn 与
...
Master 生成入口显式区分 `TurnEntry.START` 与 `TurnEntry.CONTINUE_USER_INTERACTION`。START 在尚无 active turn 时完成建议清理、
无效消息清理和附件引用精确回填，再由 `TurnCommitter.start()` 建立 owner；批准、拒绝和回答都由
`applyToolUserDecision` 提交唯一 `ResolveToolInteraction`，再只在本次 `locator.assistantMessageId` 的 assistant 上判断是否还有
Pending：有则继续等待，无则进入 CONTINUE_USER_INTERACTION，只复用既有 `TurnHandle` 并调用
`TurnCommitter.continueActive()`，不得再次执行结构预检或提交树命令。
...
`TurnStreamProjection.toolLivePhases` 是运行中工具卡片的唯一阶段投影，仍由同一 Runtime snapshot 发布。Provider 首次流出
Tool part 时为 `CALL_STREAMING`；`ModelResponseCheckpoint` 提交后转为 `READY`、`AWAITING_APPROVAL` 或 `AWAITING_INPUT`；`STARTED` execution fact
...
会话页和 Drawer 不读取 Runtime 的 coroutine Job。`ConversationPresentation` 从私有 active turn 与 durable
snapshot 派生 `TurnLivePhase`：`PREPARING`、`MODEL_WAITING`、`MODEL_STREAMING`、`TOOL_PREPARING`、`AWAITING_USER`、`TOOL_EXECUTING`、`STOPPING`（无 active presentation 即 idle）；生成子阶段由 loop 的 typed 阶段事件推进，`AWAITING_USER`/`STOPPING` 只由 durable checkpoint 与取消拥有；用户交互暂停仍属于同一 turn owner，底部使用不同颜色和问询图标保持明确的
...
`RequestContextPlanner` 负责请求窗口，`ToolOutputCompactionPlanner.planAfterSuccessfulRequest` 负责成功 Provider step 后的 Tool Result 压缩候选。完整工具结果先经
`ToolResultCheckpoint` durable checkpoint 保存，并且只有被成功 `ModelRequestReceipt` 保守确认进入最终 Provider 请求投影后，
...
全部生产阈值只来自 `ContextBudget.kt`，Provider input token 与 cache 命中百分比
...
`ModelResponseCheckpoint` 共用事务，不能只提交最后一条 Assistant。checkpoint 成功后才发布 lease 和 marker projection，失败则
...
Provider 只返回开头 `<think>` 文本时，`ThinkTagTransformer` 在 Master/Target 共用 output pipeline 以最后一个已执行 Tool 为边界，只把当前 assistant→tool step 的首个非空 `Text` 拆成 `Reasoning` 与回答正文。只有当前 step 已有 Provider 原生 Reasoning 时才禁用该 fallback；已完成 step 的 Reasoning 不得抑制后续 step。闭合标签到达时立即冻结当前 step reasoning 的 `finishedAt`，后续累计正文 chunk 只从上一投影的同一 step 复用首次闭合时间；流结束只为当前 step 未闭合标签补时间。`TurnRunner` 的 phase 判定同样基于累计 raw message 的当前 step，同一标签内文本只发布 `reasoning_streaming`，标签后的正文出现后才发布 `answer_streaming`；终态提交保留各 step 闭合标签首次投影的完成时间。
...
与恢复仍由 Runtime/TurnCommitter 唯一拥有。Android 15+ dataSync 的 `onTimeout(startId, fgsType)` 会先立即
...
正常 supersede/cancel 属于 `TurnFinalizer`，Child lineage/retention/delete 属于 `SubAssistantLifecycle`，
`TurnRecovery` 只负责进程恢复。

## 子助手扩展

子助手不是第二套生成引擎。`SubAssistantRunCoordinator` 只增加 preflight → materialize Child → run → terminal 四阶段：
...
`TurnCommitter`、`TurnRunner` 与 checkpoint 协议。共享生成实现不共享 turn usage 状态，详见
[`token-usage-accounting.md`](token-usage-accounting.md)。
...
| Master turn 编排 | `app/src/main/java/net/weero/measix/pilot/service/ConversationTurnService.kt`、`GenerationSideEffects.kt`、`ConversationTitleCoordinator.kt` |
| 命令与 Runtime 主链 | `app/src/main/java/net/weero/measix/pilot/service/runtime/ConversationCommands.kt`、`ConversationTransition.kt`、`ConversationRuntime.kt`、`ConversationRuntimeRegistry.kt`、`ConversationCommandCoordinator.kt` |
| UI 只读投影 | `app/src/main/java/net/weero/measix/pilot/service/runtime/ConversationPresentation.kt` |
| 终态与恢复 | `app/src/main/java/net/weero/measix/pilot/service/turn/TurnFinalizer.kt`、`TurnRecovery.kt`；`app/src/main/java/net/weero/measix/pilot/service/ApplicationRecoveryCoordinator.kt` |
| Provider 循环与提交 | `app/src/main/java/net/weero/measix/pilot/service/turn/TurnRunner.kt`（多 Step 循环）、`StepRunner.kt`（单 Step 采样）、`ToolBatchRunner.kt`（工具批次门控与串行执行）、`TurnRunState.kt`（跨 Step 运行状态与 checkpoint/投影写协议）、`TurnCommitter.kt`（checkpoint/终态命令适配） |
| 工具装配 | `app/src/main/java/net/weero/measix/pilot/data/ai/tools/TurnToolSetFactory.kt` |
...
| 子助手扩展 | `app/src/main/java/net/weero/measix/pilot/service/subassistant/SubAssistantRunCoordinator.kt`、`service/subassistant/SubAssistantLifecycle.kt` |
