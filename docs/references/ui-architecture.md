...
`EnterpriseExperienceDialog` 是企业空间与助手目录的直接查看入口，公开配置与记忆分别呈现；提示词和 Seed 可展开，运行记忆复用 `AssistantMemoryContent`。
工作台原生宿主另提供 Starter 列表和预览，打开操作固定在滚动内容外；路由按既有编码传递文本并保留原聊天导航项，网页 Bridge 权限不变。
共享定义说明位于助手、搜索、提示词和快捷消息页面；语音页面区分当前空间默认资源与共享服务编辑。空间身份只在关键入口显示少量图标，不在每项设置重复标记。
