...
| 图片任务与交接 | `ImageGenerationCoordinatorTest` 验证原页面 lease、取消准备/失败释放/退出重试、重复 ID 隔离、Tool 借用及冻结企业 profile 在 I/O 前拒绝编辑/数量/尺寸/partial；`SafeRoutedImageDownloaderTest` 验证企业 URL 结果的 HTTPS、公网逐跳/DNS 固定、无凭据下载、上限与 MIME/签名；`GeneratedMediaStoreTest` 验证图库提交与副本接收失败、取消回交；`ImgGenVMTest` 验证连续替换等待清理及企业 profile UI；`EnterpriseImageGenerationAndroidTest` 使用实际 DataStore、Room、原生 PNG 和生产队列验证域隔离、旧页面拒绝及退出，真实 Core/Provider 网络另验 |
