...
class PortalWebViewAndroidTest {
    ...
    fun deliveredPortalLogoutRequiresNativeConfirmationAndCompletesWithoutPageReply() = runBlocking<Unit> {
        ...
        try {
            ...
            val exit = EnterpriseExitService(sessions, sync, conversations,
                ApplicationRecoveryGate().apply { ready() }, scope, registry, mockk(relaxed = true), mockk { io.mockk.coEvery { closeRealm(any()) } returns Unit }, mcp = mockk { io.mockk.coEvery { closeRealm(any()) } returns Unit }, speech = mockk(relaxed = true))
    ...
    fun deliveredPortalCapturesAndPreviewsActualCameraAndMicrophoneMedia() = runBlocking<Unit> {
        ...
        try {
            ...
            val exit = EnterpriseExitService(sessions, sync, mockk(), ApplicationRecoveryGate().apply { ready() }, scope, registry, mockk(relaxed = true), mockk { io.mockk.coEvery { closeRealm(any()) } returns Unit }, mcp = mockk { io.mockk.coEvery { closeRealm(any()) } returns Unit }, speech = mockk(relaxed = true))
            lateinit var native: PortalNativeActions
            var displayed by mutableStateOf<PortalWebView?>(null)
            val host = withContext(Dispatchers.Main) {
