...
class ConversationRepositoryTreeIntegrationTest {
    ...
    fun enterpriseConfigurationReferencesRoundTripWithoutChangingConversationOrMessageIds() = runBlocking {
        val scope = ConfigurationScope.Enterprise(EnterpriseAuthority("dep_example"), "alice")
        val assistantId = ConfigurationReference.parse("managed~dep_example~assistant_review")
        val modelId = ConfigurationReference.parse("managed~dep_example~mdl_chat")
    ...
    fun unfinishedTurnCountIncludesChildrenAndIsolatesCompleteEnterpriseIdentity() = runBlocking {
        ...
        val scopes = listOf(ConfigurationScope.Personal, alice, alice.copy(userId = "bob"),
            alice.copy(authority = EnterpriseAuthority("dep_other")))
