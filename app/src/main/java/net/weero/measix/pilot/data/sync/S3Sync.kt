    ...
    suspend fun prepareBackupFile(config: S3Config): File = withContext(Dispatchers.IO) {
        ...
        ZipOutputStream(FileOutputStream(backupFile)).use { zipOut ->
            ...
            if (config.items.contains(S3Config.BackupItem.DATABASE)) {
                val dbFile = context.getDatabasePath("measix_pilot")
                if (dbFile.exists()) {
                    addFileToZip(zipOut, dbFile, "measix_pilot.db")
                }

                val walFile = File(dbFile.parentFile, "measix_pilot-wal")
                if (walFile.exists()) {
                    addFileToZip(zipOut, walFile, "measix_pilot-wal")
                }

                val shmFile = File(dbFile.parentFile, "measix_pilot-shm")
                if (shmFile.exists()) {
                    addFileToZip(zipOut, shmFile, "measix_pilot-shm")
                }
            }
            ...
            while (zipIn.nextEntry.also { entry = it } != null) {
                entry?.let { zipEntry ->
                    ...
                    when (zipEntry.name) {
                        ...
                        "measix_pilot.db", "measix_pilot-wal", "measix_pilot-shm" -> {
                            if (config.items.contains(S3Config.BackupItem.DATABASE)) {
                                val dbFile = when (zipEntry.name) {
                                    "measix_pilot.db" -> context.getDatabasePath("measix_pilot")
                                    "measix_pilot-wal" -> File(
                                        context.getDatabasePath("measix_pilot").parentFile,
                                        "measix_pilot-wal"
                                    )

                                    "measix_pilot-shm" -> File(
                                        context.getDatabasePath("measix_pilot").parentFile,
                                        "measix_pilot-shm"
                                    )
