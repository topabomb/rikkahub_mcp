...
data class ArtifactMediaPreview(val artifactId: Long, val uri: String, val displayName: String? = null, val modifiedAtMillis: Long? = null)
...
class ArtifactStore(
    ...
    private suspend fun readImagePayload(entity: ArtifactEntity): ByteArray {
        val bytes = payloadStore.readBytes(entity.relativePath, GeneratedMediaStore.MAX_IMAGE_BYTES.toLong())
        check(!ImageMime.isUnsupportedNonImage(bytes, entity.mimeType) && ImageMime.isAcceptedImage(bytes)) { "artifact_image_invalid" }
        return bytes
    }
    ...
    private suspend fun resolveActivePreview(
        ...
        ArtifactMediaPreview(entity.id, AttachmentRefs.fileToFileUrl(file), entity.displayName, entity.updatedAt)
    }
    ...
    internal suspend fun updateAssistantPreferenceReferences(
        scope: ConfigurationScope,
    ...
    suspend fun deleteUserRequestedFolderCreatedBefore(
        ...
        val result = deleteEntitiesLocked(entities, detach)
    ...
    internal suspend fun clearEnterpriseScope(scope: ConfigurationScope.Enterprise) = withSettingsDetach { detach ->
        ...
        check(result.cleanupPending == 0 && result.skippedInProgress == 0 && result.failed == 0) {
            "enterprise_artifact_cleanup_pending"
        }
    }

    private suspend fun deleteEntitiesLocked(entities: List<ArtifactEntity>, detach: suspend (Set<String>) -> Boolean): ArtifactCleanupResult {
        var deleted = 0
        ...
        entities.forEach { entity ->
            ...
            val result = when (entity.state) {
                ArtifactState.ACTIVE.name -> deleteUserRequestedLocked(entity.scope, entity.id, detach)
        ...
        return ArtifactCleanupResult(deleted = deleted, cleanupPending = cleanupPending, skippedInProgress = skippedInProgress, failed = failed)
    }
