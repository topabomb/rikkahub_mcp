package net.weero.mersix.pilot.data.sync
...
import net.weero.mersix.pilot.data.files.FileFolders
import net.weero.mersix.pilot.data.files.SkillPaths
import net.weero.mersix.pilot.data.datastore.Settings
import net.weero.mersix.pilot.data.datastore.SettingsStore
import net.weero.mersix.pilot.data.sync.s3.S3Client
import net.weero.mersix.pilot.data.sync.s3.S3Config
import net.weero.mersix.pilot.utils.fileSizeToString
import java.io.File
...
class S3Sync(
    ...
    suspend fun backupToS3(config: S3Config) = withContext(Dispatchers.IO) {
        ...
        val client = getS3Client(config)
        val key = "mersix_pilot_backups/${file.name}"
    ...
    suspend fun listBackupFiles(config: S3Config): List<S3BackupItem> = withContext(Dispatchers.IO) {
        ...
        val result = client.listObjects(
            prefix = "mersix_pilot_backups/",
            maxKeys = 1000
        ).getOrThrow()

        result.objects
            .filter { it.key.startsWith("mersix_pilot_backups/backup_") && it.key.endsWith(".zip") }
            .map { obj ->
                S3BackupItem(
                    key = obj.key,
    ...
    suspend fun prepareBackupFile(config: S3Config): File = withContext(Dispatchers.IO) {
        ...
        ZipOutputStream(FileOutputStream(backupFile)).use { zipOut ->
            ...
            if (config.items.contains(S3Config.BackupItem.DATABASE)) {
                val dbFile = context.getDatabasePath("mersix_pilot")
                if (dbFile.exists()) {
                    addFileToZip(zipOut, dbFile, "mersix_pilot.db")
                }

                val walFile = File(dbFile.parentFile, "mersix_pilot-wal")
                if (walFile.exists()) {
                    addFileToZip(zipOut, walFile, "mersix_pilot-wal")
                }

                val shmFile = File(dbFile.parentFile, "mersix_pilot-shm")
                if (shmFile.exists()) {
                    addFileToZip(zipOut, shmFile, "mersix_pilot-shm")
                }
            }
            ...
            while (zipIn.nextEntry.also { entry = it } != null) {
                entry?.let { zipEntry ->
                    ...
                    when (zipEntry.name) {
                        ...
                        "mersix_pilot.db", "mersix_pilot-wal", "mersix_pilot-shm" -> {
                            if (config.items.contains(S3Config.BackupItem.DATABASE)) {
                                val dbFile = when (zipEntry.name) {
                                    "mersix_pilot.db" -> context.getDatabasePath("mersix_pilot")
                                    "mersix_pilot-wal" -> File(
                                        context.getDatabasePath("mersix_pilot").parentFile,
                                        "mersix_pilot-wal"
                                    )

                                    "mersix_pilot-shm" -> File(
                                        context.getDatabasePath("mersix_pilot").parentFile,
                                        "mersix_pilot-shm"
                                    )
