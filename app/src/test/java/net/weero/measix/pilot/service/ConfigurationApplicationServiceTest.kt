...
class ConfigurationApplicationServiceTest {
    ...
    fun `scoped write persists only a preference and personal editing retains it`() = runTest {
        ...
        try {
            ...
            env.commands.updateAssistantUsage(packet.identity.scope, env.assistant.id) {
    ...
    fun `policy change waits for an owned preference transaction and prevents a later forbidden selection`() = runTest {
        ...
        try {
            ...
            try {
                env.commands.updateAssistantUsage(packet.identity.scope, env.assistant.id) { it!!.copy(mcpServers = UsageValue(setOf(env.mcp.id))) }
    ...
    fun `same enterprise different user has independent usage and wrong principal cannot write`() = runTest {
        ...
        try {
            ...
            env.commands.updateAssistantUsage(alice.identity.scope, env.assistant.id) {
            ...
            try {
                ...
                assertEquals("enterprise_principal_mismatch", error.reason)
            }
    ...
    fun `realm resource choices retain invalid references and reject replacements that do not satisfy policy or capability`() = runTest {
        ...
        try {
            ...
            commands.selectResource(scope, ResourceSelectionSlot.CHAT_MODEL, env.model.id)
            commands.selectResource(scope, ResourceSelectionSlot.ASSISTANT, env.assistant.id)
            ...
            commands.setSuggestionEnabled(scope, false)
            commands.setModelFavorite(scope, env.model.id, false)
            commands.selectResource(scope, ResourceSelectionSlot.CHAT_MODEL, null)
            ...
            for (reference in listOf(env.model.id, packet.identity.reference("mdl_image"))) {
                try {
                    commands.selectResource(scope, ResourceSelectionSlot.CHAT_MODEL, reference)
            ...
            commands.selectResource(scope, ResourceSelectionSlot.IMAGE_MODEL, packet.identity.reference("mdl_image"))
    ...
    fun `preference commit failure leaves durable choices and query unchanged and releases authorization lock`() = runTest {
        ...
        try {
            ...
            try {
                env.commands.selectResource(scope, ResourceSelectionSlot.CHAT_MODEL, env.model.id)
    ...
    fun `cancellation during actual preference write holds authorization until commit then permits session exit`() = runTest {
        ...
        try {
            ...
            val writer = launch {
                env.commands.updateAssistantUsage(scope, env.assistant.id) {
                    AssistantUsagePreferences(env.assistant.id, temperature = UsageValue(0.2f))
                }
            }
            entered.await()
    ...
    fun `cancellation during preference transform abandons the mutation before writer handoff`() = runTest {
        ...
        try {
            ...
            val writer = launch {
                ...
                env.commands.updateAssistantUsage(exampleEnterprisePackage().identity.scope, env.assistant.id) {
                    caller.cancel()
                    AssistantUsagePreferences(env.assistant.id, temperature = UsageValue(0.2f))
                }
            }
